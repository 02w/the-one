...
/**
 * Superclass for all movement models. All subclasses must contain at least a constructor with one
 * {@link Settings} parameter and also a copy-constructor. They must also implement the {@link
 * #replicate()} method, which should return an instance of the movement model class with same
 * parameters as the object (immutable fields can be shared, but mutable fields must be copied).
 *
 * <p>To make a new movement model do something useful, also at least {@link #getInitialLocation()}
 * and {@link #getPath()} are worthwhile to override.
 */
public abstract class MovementModel {
  ...
  /** MovementModel namespace (where world size and rng seed settings are looked from ({@value}) */
  ...
  public MovementModel(Settings settings) {
    ...
    if (settings.contains(MovementModel.SPEED)) {
      speeds = settings.getCsvDoubles(MovementModel.SPEED, 2);
    } else {
      speeds = MovementModel.DEF_SPEEDS;
    }

    this.minSpeed = speeds[0];
    this.maxSpeed = speeds[1];
    MovementModel.checkMinAndMaxSetting(MovementModel.SPEED, this.minSpeed, this.maxSpeed);

    if (settings.contains(MovementModel.WAIT_TIME)) {
      times = settings.getCsvDoubles(MovementModel.WAIT_TIME, 2);
    } else {
      times = MovementModel.DEF_WAIT_TIMES;
    }

    this.minWaitTime = times[0];
    this.maxWaitTime = times[1];
    MovementModel.checkMinAndMaxSetting(MovementModel.WAIT_TIME, this.minWaitTime, this.maxWaitTime);

    settings.setNameSpace(MovementModel.MOVEMENT_MODEL_NS);
    int[] worldSize = settings.getCsvInts(MovementModel.WORLD_SIZE, 2);
  ...
  public static void reset() {
    ...
    if (s.contains(MovementModel.RNG_SEED)) {
      ...
      MovementModel.rng = new Random(seed);
    } else {
      MovementModel.rng = new Random(0);
    }
  }
  ...
  /**
   * Generates and returns a speed value between min and max of the {@link #WAIT_TIME} setting.
   *
   * @return A new speed between min and max values
   */
  protected double generateSpeed() {
    if (MovementModel.rng == null) {
      return 1;
    }
    return (this.maxSpeed - this.minSpeed) * MovementModel.rng.nextDouble() + this.minSpeed;
  }

  /**
   * Generates and returns a suitable waiting time at the end of a path. (i.e. random variable whose
   * value is between min and max of the {@link #WAIT_TIME} setting).
   *
  ...
  protected double generateWaitTime() {
    ...
    return (this.maxWaitTime - this.minWaitTime) * MovementModel.rng.nextDouble() + this.minWaitTime;
  }

  /**
   * Returns a new path by this movement model or null if no new path could be constructed at the
   * moment (node should wait where it is). A new path should not be requested before the
   * destination of the previous path has been reached.
   *
  ...
  /**
   * Returns a sim time when the next path is available. This implementation returns a random time
   * in future that is {@link #WAIT_TIME} from now.
   *
   * @return The sim time when node should ask the next time for a path
   */
  public double nextPathAvailable() {
    return SimClock.getTime() + this.generateWaitTime();
  }
