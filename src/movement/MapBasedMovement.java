...
public class MapBasedMovement extends MovementModel implements SwitchableMovement {
  ...
  /**
   * Per node group setting for selecting map node types that are OK for this node group to traverse
   * trough. Value must be a comma separated list of integers in range of [1,31]. Values reference
   * to map file indexes (see {@link #FILE_S}). If setting is not defined, all map nodes are
   * considered OK.
   */
  ...
  public MapBasedMovement(Settings settings) {
    ...
    this.readOkMapNodeTypes(settings);
  ...
  private void readOkMapNodeTypes(Settings settings) {
    if (settings.contains(MapBasedMovement.MAP_SELECT_S)) {
      this.okMapNodeTypes = settings.getCsvInts(MapBasedMovement.MAP_SELECT_S);
      for (int i : this.okMapNodeTypes) {
        if (i < MapNode.MIN_TYPE || i > MapNode.MAX_TYPE) {
          throw new SettingsError(
              "Map type selection '"
                  + i
                  + "' is out of range for setting "
                  + settings.getFullPropertyName(MapBasedMovement.MAP_SELECT_S));
        }
        if (i > this.nrofMapFilesRead) {
          throw new SettingsError(
  ...
  public Coord getInitialLocation() {
    List<MapNode> nodes = this.map.getNodes();
    MapNode n, n2;
    Coord n2Location, nLocation, placement;
    double dx, dy;
    double rnd = MovementModel.rng.nextDouble();

    // choose a random node (from OK types if such are defined)
    do {
      n = nodes.get(MovementModel.rng.nextInt(nodes.size()));
    } while (this.okMapNodeTypes != null && !n.isType(this.okMapNodeTypes));

    // choose a random neighbor of the selected node
    n2 = n.getNeighbors().get(MovementModel.rng.nextInt(n.getNeighbors().size()));
  ...
  /**
   * Returns map node types that are OK for this movement model in an array or null if all values
   * are considered ok
   *
   * @return map node types that are OK for this movement model in an array
   */
  protected int[] getOkMapNodeTypes() {
    return this.okMapNodeTypes;
  }
  ...
  public Path getPath() {
    ...
    MapNode prevNode = this.lastMapNode;
    MapNode nextNode = null;
    List<MapNode> neighbors;
    Coord nextCoord;

    assert this.lastMapNode != null : "Tried to get a path before placement";

    // start paths from current node
    p.addWaypoint(curNode.getLocation());

    int pathLength = MovementModel.rng.nextInt(this.maxPathLength - this.minPathLength) + this.minPathLength;
    ...
    for (int i = 0; i < pathLength; i++) {
      ...
      if (this.okMapNodeTypes != null) { // remove neighbor nodes that aren't ok
        for (int j = 0; j < n2.size(); ) {
          if (!n2.get(j).isType(this.okMapNodeTypes)) {
            n2.remove(j);
          } else {
            j++;
          }
      ...
      if (n2.size() == 0) { // only option is to go back
        ...
        nextNode = n2.get(MovementModel.rng.nextInt(n2.size()));
      }
  ...
  private SimMap readMap() {
    ...
    if (MapBasedMovement.cachedMap == null) {
      MapBasedMovement.cachedMapFiles = new ArrayList<>(); // no cache present
    } else { // something in cache
      // check out if previously asked map was asked again
      SimMap cached = this.checkCache(settings);
      if (cached != null) {
        this.nrofMapFilesRead = MapBasedMovement.cachedMapFiles.size();
        return cached; // we had right map cached -> return it
      } else { // no hit -> reset cache
        MapBasedMovement.cachedMapFiles = new ArrayList<>();
        MapBasedMovement.cachedMap = null;
      }
    }
    ...
    try {
      ...
      for (int i = 1; i <= nrofMapFiles; i++) {
        ...
        MapBasedMovement.cachedMapFiles.add(pathFile);
    ...
    simMap = r.getMap();
    this.checkMapConnectedness(simMap.getNodes());
    ...
    this.checkCoordValidity(simMap.getNodes());

    MapBasedMovement.cachedMap = simMap;
    return simMap;
  }
  ...
  private void checkMapConnectedness(List<MapNode> nodes) {
    ...
    Queue<MapNode> unvisited = new LinkedList<>();
    MapNode firstNode;
    MapNode next = null;
  ...
  private SimMap checkCache(Settings settings) {
    ...
    if (nrofMapFiles != MapBasedMovement.cachedMapFiles.size() || MapBasedMovement.cachedMap == null) {
      return null; // wrong number of files
    }

    for (int i = 1; i <= nrofMapFiles; i++) {
      String pathFile = settings.getSetting(MapBasedMovement.FILE_S + i);
      if (!pathFile.equals(MapBasedMovement.cachedMapFiles.get(i - 1))) {
        return null; // found wrong file name
      }
    }

    // all files matched -> return cached map
    return MapBasedMovement.cachedMap;
  }
  ...
  public void setLocation(Coord lastWaypoint) {
    ...
    Iterator<MapNode> iterator = this.getMap().getNodes().iterator();
