...
public abstract class MessageRouter {
  ...
  /** Message TTL -setting id ({@value}). Value is in minutes and must be an integer. */
  public static final String MSG_TTL_S = "msgTtl";
  /**
   * Message/fragment sending queue type -setting id ({@value}). This setting affects the order the
   * messages and fragments are sent if the routing protocol doesn't define any particular order
   * (e.g, if more than one message can be sent directly to the final recipient). Valid values are
   * <br>
   *
   * <UL>
   ...
   * Size of the message buffer is read from {@link #B_SIZE_S} setting. Default value is
   * Integer.MAX_VALUE.
   *
  ...
  public MessageRouter(Settings s) {
    ...
    if (s.contains(MessageRouter.B_SIZE_S)) {
      this.bufferSize = s.getLong(MessageRouter.B_SIZE_S);
    }

    if (s.contains(MessageRouter.MSG_TTL_S)) {
      this.msgTtl = s.getInt(MessageRouter.MSG_TTL_S);

      if (this.msgTtl > MessageRouter.MAX_TTL_VALUE) {
        throw new SettingsError(
            "Invalid value for "
                + s.getFullPropertyName(MessageRouter.MSG_TTL_S)
                + ". Max value is limited to "
                + MessageRouter.MAX_TTL_VALUE);
      }
    }

    if (s.contains(MessageRouter.SEND_QUEUE_MODE_S)) {

      String mode = s.getSetting(MessageRouter.SEND_QUEUE_MODE_S);

      if (mode.trim().equalsIgnoreCase(MessageRouter.STR_Q_MODE_FIFO)) {
        this.sendQueueMode = MessageRouter.Q_MODE_FIFO;
      } else if (mode.trim().equalsIgnoreCase(MessageRouter.STR_Q_MODE_RANDOM)) {
        this.sendQueueMode = MessageRouter.Q_MODE_RANDOM;
      } else {
        this.sendQueueMode = s.getInt(MessageRouter.SEND_QUEUE_MODE_S);
        if (this.sendQueueMode < 1 || this.sendQueueMode > 2) {
          throw new SettingsError("Invalid value for " + s.getFullPropertyName(
              MessageRouter.SEND_QUEUE_MODE_S));
        }
      }
    } else {
      this.sendQueueMode = MessageRouter.Q_MODE_RANDOM;
  ...
  protected MessageRouter(MessageRouter r) {
    ...
    for (Collection<Application> apps : r.applications.values()) {
      for (Application app : apps) {
        this.addApplication(app.replicate());
      }
    }
  }

  /**
   * Initializes the router; i.e. sets the host this router is in and message listeners that need to
   * be informed about message related events etc.
   *
  ...
  /**
   * Updates router. This method should be called (at least once) on every simulation interval to
   * update the status of transfer(s).
   */
  public void update() {
    for (Collection<Application> apps : this.applications.values()) {
      for (Application app : apps) {
  ...
  /**
   * Returns true if a full message with same ID as the given message has been received by this host
   * as the <strong>final</strong> recipient (at least once).
   *
   * @param m message we're interested of
   * @return true if a message with the same ID has been received by this host as the final
   *     recipient.
   */
  ...
  /**
   * Returns <code>true</code> if the message has been blacklisted. Messages get blacklisted when an
   * application running on the node wants to drop it. This ensures the peer doesn't try to
   * constantly send the same message to this node, just to get dropped by an application every
   * time.
   *
   * @param id id of the message
  ...
  /**
   * Returns a reference to the messages of this router in collection. <b>Note:</b> If there's a
   * chance that some message(s) from the collection could be deleted (or added) while iterating
   * through the collection, a copy of the collection should be made to avoid concurrent
  ...
  /**
   * Returns the amount of free space in the buffer. May return a negative value if there are more
   * messages in the buffer than should fit there (because of creating new messages).
   *
  ...
  public void sendMessage(String id, DTNHost to) {
    ...
    Message m2;
    if (m == null) {
      throw new SimError("no message for id " + id + " to send at " + this.host);
    }
   ...
   * @return Value zero if the node accepted the message (RCV_OK), value less than zero if node
   *     rejected the message (e.g. DENIED_OLD), value bigger than zero if the other node should try
   *     later (e.g. TRY_LATER_BUSY).
   */
  public int receiveMessage(Message m, DTNHost from) {
    ...
    for (MessageListener ml : this.mListeners) {
      ml.messageTransferStarted(newMessage, from, this.getHost());
    }

    return MessageRouter.RCV_OK; // superclass always accepts messages
  }

  /**
   * This method should be called (on the receiving host) after a message was successfully
   * transferred. The transferred message is put to the message buffer unless this host is the final
  ...
  public Message messageTransferred(String id, DTNHost from) {
    Message incoming = this.removeFromIncomingBuffer(id, from);
    boolean isFinalRecipient;
    boolean isFirstDelivery; // is this first delivered instance of the msg

    if (incoming == null) {
      throw new SimError(
          "No message with ID " + id + " in the incoming " + "buffer of " + this.host);
    }
    ...
    for (Application app : this.getApplications(incoming.getAppID())) {
    ...
    isFirstDelivery = isFinalRecipient && !this.isDeliveredMessage(aMessage);
  ...
  /**
   * Puts a message to incoming messages buffer. Two messages with the same ID are distinguished by
   * the from host.
   *
  ...
  /**
   * Removes and returns a message with a certain ID from the incoming messages buffer or null if
   * such message wasn't found.
   *
   * @param id ID of the message
  ...
  /**
   * Returns true if a message with the given ID is one of the currently incoming messages, false if
   * not
   *
   * @param id ID of the message
  ...
  /**
   * Adds a message to the message buffer and informs message listeners about new message (if
   * requested).
   *
   * @param m The message to add
   * @param newMessage If true, message listeners are informed about a new message, if false,
   *     nothing is informed.
   */
  ...
  public void messageAborted(String id, DTNHost from, int bytesRemaining) {
    Message incoming = this.removeFromIncomingBuffer(id, from);
    if (incoming == null) {
      throw new SimError("No incoming message for id " + id + " to abort in " + this.host);
    }
   ...
   * @return True if the creation succeeded, false if not (e.g. the message was too big for the
   *     buffer)
   */
   ...
   * @param drop If the message is dropped (e.g. because of full buffer) this should be set to true.
   *     False value indicates e.g. remove of message because it was delivered to final destination.
   */
  public void deleteMessage(String id, boolean drop) {
    Message removed = this.removeFromMessages(id);
    if (removed == null) {
  ...
  /**
   * Sorts/shuffles the given list according to the current sending queue mode. The list can contain
   * either Message or Tuple<Message, Connection> objects. Other objects cause error.
   *
  ...
  protected List sortByQueueMode(List list) {
    switch (this.sendQueueMode) {
      case MessageRouter.Q_MODE_RANDOM:
        Collections.shuffle(list, new Random(SimClock.getIntTime()));
        break;
      case MessageRouter.Q_MODE_FIFO:
        Collections.sort(
            list,
            new Comparator() {
      ...
      default:
        throw new SimError("Unknown queue mode " + this.sendQueueMode);
    }
   ...
   * @return -1 if the first message should come first, 1 if the second message should come first,
   *     or 0 if the ordering isn't defined
   */
  ...
  public RoutingInfo getRoutingInfo() {
    ...
    RoutingInfo delivered =
        new RoutingInfo(this.deliveredMessages.size() + " delivered message(s)");

    RoutingInfo cons = new RoutingInfo(this.host.getConnections().size() + " connection(s)");
    ...
    for (Connection c : this.host.getConnections()) {
  ...
  public void addApplication(Application app) {
    if (!this.applications.containsKey(app.getAppID())) {
      this.applications.put(app.getAppID(), new LinkedList<>());
    }
  ...
  /**
   * Creates a replicate of this router. The replicate has the same settings as this router but
   * empty buffers and routing tables.
   *
