...
public class SimScenario implements Serializable {
  ...
  protected SimScenario() {
    ...
    this.nrofGroups = s.getInt(SimScenario.NROF_GROUPS_S);

    this.name = s.valueFillString(s.getSetting(SimScenario.NAME_S));
    this.endTime = s.getDouble(SimScenario.END_TIME_S);
    this.updateInterval = s.getDouble(SimScenario.UP_INT_S);
    this.simulateConnections = s.getBoolean(SimScenario.SIM_CON_S);

    s.ensurePositiveValue(this.nrofGroups, SimScenario.NROF_GROUPS_S);
    s.ensurePositiveValue(this.endTime, SimScenario.END_TIME_S);
    s.ensurePositiveValue(this.updateInterval, SimScenario.UP_INT_S);
    ...
    this.world =
        new World(
            ...
            this.eqHandler.getEventQueues());
  }

  public static void reset() {
    SimScenario.myinstance = null;
  }

  /** Returns the SimScenario instance and creates one if it doesn't exist yet */
  public static SimScenario getInstance() {
    if (SimScenario.myinstance == null) {
      SimScenario.myinstance = new SimScenario();
    }
    return SimScenario.myinstance;
  }
  ...
  protected void createHosts() {
    ...
    for (int i = 1; i <= this.nrofGroups; i++) {
      ...
      Settings s = new Settings(SimScenario.GROUP_NS + i);
      ...
      int nrofInterfaces = s.getInt(SimScenario.NROF_INTERF_S);
      int appCount;

      // creates prototypes of MessageRouter and MovementModel
      MovementModel mmProto =
          (MovementModel) s.createIntializedObject(SimScenario.MM_PACKAGE + s.getSetting(
              SimScenario.MOVEMENT_MODEL_S));
      MessageRouter mRouterProto =
          (MessageRouter) s.createIntializedObject(SimScenario.ROUTING_PACKAGE + s.getSetting(
              SimScenario.ROUTER_S));

      /* checks that these values are positive (throws Error if not) */
      s.ensurePositiveValue(nrofHosts, SimScenario.NROF_HOSTS_S);
      s.ensurePositiveValue(nrofInterfaces, SimScenario.NROF_INTERF_S);

      // setup interfaces
      for (int j = 1; j <= nrofInterfaces; j++) {
        String intName = s.getSetting(SimScenario.INTERFACENAME_S + j);
        Settings intSettings = new Settings(intName);
        NetworkInterface iface =
            (NetworkInterface)
                intSettings.createIntializedObject(
                    SimScenario.INTTYPE_PACKAGE + intSettings.getSetting(SimScenario.INTTYPE_S));
        iface.setClisteners(this.connectionListeners);
        iface.setGroupSettings(s);
        interfaces.add(iface);
      }

      // setup applications
      if (s.contains(SimScenario.APPCOUNT_S)) {
        appCount = s.getInt(SimScenario.APPCOUNT_S);
      } else {
        appCount = 0;
      }
      for (int j = 1; j <= appCount; j++) {
        ...
        try {
          ...
          appname = s.getSetting(SimScenario.GAPPNAME_S + j);
          // Get settings for the given application
          Settings t = new Settings(appname);
          // Load an instance of the application
          protoApp = (Application) t.createIntializedObject(SimScenario.APP_PACKAGE + t.getSetting(
              SimScenario.APPTYPE_S));
